/*
 * A „Méret és fonal” szakasz szövegei (PQW-859): érték eredettel, a becslés
 * tartománnyal, profil nélkül egyértelmű jelzés, tűméret-átváltás, fonal
...
test('a tű amerikai és régi brit mérete a mm mellett; nem szabványosnál a legközelebbi', () => {
...
test('érték eredettel, magyar tizedesvesszővel; a becslés ≈ jellel és tartománnyal', () => {
  ...
  // Ha a két határ kerekítve egyezik, nincs külön tartomány.
  assert.equal(quantityText(estimate(1, [1, 1.2]), 'db', 0).range, null);
  assert.equal(formatNumber(1804.4, 0), '1804');
});

test('profil nélkül a szakasz kimondja, hogy a méret becslés, és minden értéknél ott a tartomány', () => {
...
test('mért profillal nincs figyelmeztetés; a fonal gombolyagra kerekítve', () => {
...
test('részben mért profilnál megmondja, mi a becslés', () => {
...
test('a profil mezőinek eredete: címkéről, mért, vagy a m/100 g-ből becsült vastagság', () => {
...
test('a hiányos gauge-sor a becslést mutatja, a kitöltött nem', () => {
