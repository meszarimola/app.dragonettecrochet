...
describe('round schedule (04 §9.4)', () => {
  test('round 1 has 2L + 2 stitches over L chains, then +6 per round; chains come from the length, rounds from the width', () => {
    for (const [lengthCm, widthCm] of [
      ...
      assert.ok(
        counts.slice(1).every((count, i) => count - counts[i] === 6),
        `${lengthCm} × ${widthCm}: +6 per round`,
      );
  ...
  test('length must be the larger dimension, and an invalid size says why', () => {
    ...
    assert.deepEqual(shapeSchedule(oval(Number.NaN, 4), DK).reason, {
      code: 'size-range',
      data: { field: 'length', max: 100 },
    });
...
describe('graph of the generated oval', () => {
  test('round 1 works into every chain on the front, 4 sc into the farthest chain, then back along the other side with 3 sc into the chain nearest the hook', () => {
    ...
    for (const id of base.positions.slice(1, -1))
      assert.deepEqual([into('stitch', id), into('underside', id)], [1, 1], id);
    ...
    for (const base of [emptyPattern(), dk]) {
      for (const shape of [oval(8, 5), oval(15, 12), oval(5, 1)]) {
        for (const stagger of [true, false]) {
          ...
          assert.deepEqual(
            computeLayers(pattern, libraryFor(pattern))
              .slice(1)
...
describe('written pattern and reading it back', () => {
  test('round 1 reads as a skip, the stitches into the front of the chain, then back along the other side', () => {
    ...
    assert.match(
      textOf(pattern, 'en-US'),
      /Rnd 1: skip 1 ch, 6 sc, 4 sc in next ch, working back along the other side of the chain: 5 sc, 3 sc in next ch \(18\)\./,
    );
    assert.match(
      textOf(pattern, 'en-GB'),
      /Rnd 1: miss 1 ch, 6 dc, 4 dc in next ch, working back along the other side of the chain: 5 dc, 3 dc in next ch \(18\)\./,
    );
  });
...
describe('the validator on round 1 of an oval', () => {
  ...
  const withAnchor = (pattern, id, anchors) => ({
    ...
    pieces: [
      {
        ...pattern.pieces[0],
        stitches: pattern.pieces[0].stitches.map((node) => (node.id === id ? { ...node, anchors } : node)),
      },
    ],
  });
  ...
  test('leaving out one chain on the other side reports an unused position', () => {
    ...
    const graph = graphOf(pattern);
    const [base, first] = graph.layers;
    const back = first.stitches.find(
      (id) => graph.nodes.get(id).anchors[0]?.into === 'underside' && !graph.groupOf.has(id),
    );
    ...
    assert.deepEqual(
      [...found].filter((rule) => rule !== 'unused-position' && rule !== 'unmarked-increase'),
      [],
    );
  });
  ...
  test('swapping the targets of two stitches on the front works against the direction of travel', () => {
    ...
    const graph = graphOf(pattern);
    const first = graph.layers[1];
    const fronts = first.stitches.filter(
      (id) => graph.nodes.get(id).anchors[0]?.into === 'stitch' && !graph.groupOf.has(id),
    );
    const [a, b] = fronts;
...
describe('layout and parts', () => {
  test('the layout draws the foundation chain straight, with the front and underside stitches of round 1 on opposite sides of it', () => {
    ...
    const side = (kind) =>
      first.stitches
        .filter((id) => graph.nodes.get(id).anchors[0]?.into === kind && !graph.groupOf.has(id))
        .map((id) => Math.sign(layout.nodes.get(id).top.y));
  ...
  test('as a part: sewn to a sphere as a sole, and a wall continued from the edge of the oval', () => {
    const head = ok(
      createAmigurumi(emptyPattern(), part({ kind: 'sphere', diameterCm: 6, method: '6n' }, { name: 'Fej' }), false),
    );
    const sewn = ok(
      addAmigurumiPart(head, part(oval(8, 5), { name: 'Talp' }), { method: 'sewn', distribute: true }, false),
    );
    ...
    const joined = ok(
      addAmigurumiPart(sole, part(wall, { name: 'Fal' }), { method: 'continuous', distribute: true }, false),
    );
    assert.deepEqual(rules(joined), []);
    assert.deepEqual(
      joined.pieces[0].sections.map((section) => section.name),
      ['Talp', 'Fal'],
    );
...
describe('half double and double crochet ovals (PQW-899)', () => {
  test('each end increases by half the flat-circle value: +8 per round for hdc, +12 for dc; the chain count includes the turning chain of the stitch', () => {
    ...
    for (const [stitch, perEnd, turningChain] of [
      ...
      assert.ok(
        counts.slice(1).every((count, i) => count - counts[i] === 2 * perEnd),
        `${stitch}: +${2 * perEnd} per round`,
      );
    }
  ...
  test('treble crochet oval (PQW-902): 8 increases per end over a 4-chain turning chain, and it validates clean', () => {
    ...
    assert.ok(
      counts.slice(1).every((count, i) => count - counts[i] === 16),
      counts.join(','),
    );
    ...
    assert.match(
      textOf(pattern),
      /1\. kör: hagyj ki 4 láncszemet, majd \d+ krp, 9 krp a következő láncszembe, a láncszemek másik oldalán vissza: /,
    );
  });
  ...
  test('the written pattern skips the turning chain of the stitch, and reads back to the same graph in all three notations', () => {
    ...
    assert.ok(
      textOf(dc).includes(
        '1. kör: hagyj ki 3 láncszemet, majd 6 erp, 7 erp a következő láncszembe, a láncszemek másik oldalán vissza: 5 erp, 5 erp a következő láncszembe (24).',
      ),
      textOf(dc),
    );
    for (const base of [emptyPattern(), withTradition('japanese')]) {
      for (const stitch of ['hdc', 'dc']) {
        ...
        for (const locale of ['hu', 'en-US', 'en-GB']) {
          ...
          assert.deepEqual(
            canonicalPattern(back.pattern).pieces,
  ...
  test('saving keeps the stitch, an older save without one loads as single crochet, and an unknown stitch is rejected', () => {
    ...
    const loaded = loadPattern(savePattern(pattern));
    assert.ok(loaded.ok);
    assert.deepEqual(loaded.pattern.pieces[0].sections[0].shape, {
      kind: 'oval',
      lengthCm: 8,
      widthCm: 5,
      stitch: 'hdc',
  ...
  test('figure height: a flat oval contributes the fabric thickness, not a round height', () => {
    ...
    const head = ok(
      createAmigurumi(base, part({ kind: 'sphere', diameterCm: 6, method: '6n' }, { name: 'Test' }), false),
    );
    const figure = figureSize(
      ok(addAmigurumiPart(head, part(oval(8, 5), { name: 'Talp' }), { method: 'sewn', distribute: true }, false)),
    );
...
describe('crocheting into the other side of the chain by hand (PQW-899)', () => {
  ...
  const cursorOf = (pattern, stitch = 'sc', editorMode = mode) =>
    defaultCursor(pattern, contextOf(pattern, editorMode), stitch);
  const place = (pattern, stitch = 'sc') =>
    ok(work(pattern, { def: stitch, count: 1 }, cursorOf(pattern, stitch), [], mode));
  ...
  test('underside targets appear after the first stitch, without the farthest chain, and the cursor jumps there once the end is worked', () => {
    ...
    assert.deepEqual(
      undersides.map((slot) => slot.id),
      base.positions.slice(1),
    );
  ...
  test('the foundation-chain start is not flagged mid-crochet: the round goes into the chain after the turning chain', () => {
    ...
    assert.deepEqual(
      liveCheck(pattern, contextOf(pattern)).findings.map((finding) => finding.rule),
      ['foundation-chain'],
    );
  });
  ...
  test('fill row continues along the other side of the chain once it passes the farthest chain', () => {
    ...
    assert.deepEqual(
      [
        anchors.filter((anchor) => anchor.into === 'stitch').length,
        anchors.filter((anchor) => anchor.into === 'underside').length,
      ],
      [7, 6],
    );
  ...
  test('in the layout an underside target sits on the far side of the chain, opposite the stitch worked into its front', () => {
    ...
    const index = context.slots.findIndex(
      (slot) => slot.kind === 'underside' && slot.id === graphOf(pattern).layers[0].positions[5],
    );
    ...
    const front = layout.nodes.get(
      pattern.pieces[0].stitches.find(
        (node) => node.anchors[0]?.id === context.slots[frontIndex].id && node.anchors[0].into === 'stitch',
      ).id,
    ).top;
  ...
  test('copying: round 1 alone is refused with a reason; together with the foundation chain it pastes into an empty pattern but cannot be duplicated', () => {
    ...
    assert.equal(
      duplicateSelection(pattern, [...base.stitches, ...first.stitches]).reason.code,
