 ...
 */
test('the label of the next row stands beside the chart, with an arrow, and covers nothing (PQW-929)', async ({ page }) => {
  await open(page);
  ...
  for (const stitch of stitches) {
    expect(overlaps(next!, stitch), `the label ${describe(next!)} covers stitch ${stitch.id} ${describe(stitch)}`).toBe(false);
  }
...
test('beside every row there is the row number and the stitch count, without covering anything (PQW-916)', async ({ page }) => {
  await open(page);
  ...
  expect(foundation!.text, 'the foundation chain is row 1, with the chain stitches drawn').toMatch(/^1\. sor – alapsor \(\d+\)$/);
  ...
  for (const label of labels) {
    ...
    expect(besideChart || hugsEdge, `the label „${label.text}” is squeezed in among the symbols ${describe(label)}`).toBe(true);
  }

  for (const label of labels) {
    for (const stitch of stitches) {
      expect(overlaps(label, stitch), `label ${label.layer} ${describe(label)} covers stitch ${stitch.id} ${describe(stitch)}`).toBe(false);
    }
...
for (const viewport of [
  ...
  test(`${viewport.width}×${viewport.height}: after „Egész minta” the labels stay between the two sidebars (PQW-916)`, async ({ page }) => {
    ...
    for (const label of labels) {
      expect(label.left, `the label „${label.text}” slides under the pattern type bar ${describe(label)}`).toBeGreaterThanOrEqual(types.x + types.width - 0.5);
      expect(label.right, `the label „${label.text}” slides under the stitch palette panel ${describe(label)}`).toBeLessThanOrEqual(panel.x + 0.5);
    }
  });
}
 ...
 */
test('1000×506, English interface: even the longer label does not slide onto the stitches (PQW-916)', async ({ page }) => {
  ...
  for (const label of labels) {
    for (const stitch of stitches) {
      expect(overlaps(label, stitch), `the label „${label.text}” ${describe(label)} covers stitch ${stitch.id} ${describe(stitch)}`).toBe(false);
    }
    if (arrow) expect(overlaps(label, arrow), `the label „${label.text}” covers the arrow ${describe(arrow)}`).toBe(false);
  }
});
...
test('there is no floating status text over the canvas area, but the live region stays (PQW-916)', async ({ page }) => {
  ...
  expect(area, `the status text still floats over the canvas: ${box ? describe({ left: box.x, top: box.y, right: box.x + box.width, bottom: box.y + box.height }) : 'it has no box'}`).toBeLessThanOrEqual(4);
});
